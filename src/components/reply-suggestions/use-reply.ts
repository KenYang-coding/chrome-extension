    ...
    try {
      ...
      await fetchEventSource(process.env.PUBLIC_API_URL + '/v2/conversation/message', {
        ...
        onmessage(ev) {
          ...
          if (resData?.code < 0) throw new Error(resData?.message);
          else if (resData?.code >= 20000) throw new Error(resData?.message);
          if (globalState.config.response_mode === 'streaming') {
            const code = resData?.code;
            if ([3, 12, 13, 14, 15, 39].includes(code)) setTimeout(() => (replySuggestion.loading = false), 300);
            if ([3, 12, 13, 14, 15].includes(code)) {
              setTimeout(() => {
                if (name !== globalState.userInfo.name) return;
                const isSameCode = replySuggestion.messages[replySuggestion.messages.length - 1]?.code === code;
                if (isSameCode) replySuggestion.messages[replySuggestion.messages.length - 1].data += resData.data;
                else replySuggestion.messages.push({ code, data: resData.data });
              }, 300);
            } else if (code === 39) {
              setTimeout(() => {
                if (name !== globalState.userInfo.name) return;
                const isSameCode = replySuggestion.messages[replySuggestion.messages.length - 1]?.code === code;
                if (isSameCode) replySuggestion.messages[replySuggestion.messages.length - 1].data += resData.data.transcript;
                else replySuggestion.messages.push({ code, data: resData.data.transcript });
              }, 300);
            } else if (code === 11) {
