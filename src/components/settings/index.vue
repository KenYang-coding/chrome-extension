...
.settings-card {
  .config-wrapper {
    ...
    .config-value { color: rgba(37, 48, 68, 0.52); }

    .config-content {
      &+.config-label { margin-top: 8px; }

      :deep(.ant-radio-wrapper) {
        margin-right: 0;
        color: rgba(37, 48, 68, 0.84);
        &+.ant-radio-wrapper { margin-left: 36px; }
      }
    }
  }
  ...
  .integrate-btn-wrapper {
    ...
    .tips {
      ...
      &.success { color: @success-color; &::before { background: @success-color; } }
      &.error { color: @error-color; &::before { background: @error-color; } }
      &.static { color: rgba(37, 48, 68, 0.84); }
    }
  }

  .iconfont.icon-fuzhi { cursor: pointer; }
}
</style>
